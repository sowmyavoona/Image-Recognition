function mappedData = gdaSir(data,trainData,trainLabel,nDim,options)
...
for p = 1:c
    for q = 1:c
        ...
        for i = 1:nSample(p)
            for j = 1:nSample(q)
                Kpq(i,j) = kernelSir(classP(:,i),classQ(:,j),options);
            end
        end
        kTrainCell{p,q} = Kpq;
    end
...
[mmm, index] = sort(diagonal,'descend');
...
for p = 1:c
    ...
    for i = 1:nSample(p)
        for j = 1:nPrime
            Kp(i,j) = kernelSir(classP(:,i),data(:,j),options);
        end
    end
    kDataCell{p,1} = Kp;
end
